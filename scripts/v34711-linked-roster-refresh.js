...
// Shared GitHub/web releases deliberately keep public-link roster reads paused.
// The Android bridge remains independent; changing this one flag and the Worker
// BLABLA_PUBLIC_SYNC_ENABLED variable is the intentional future re-open path.
const PUBLIC_WEB_BLABLA_SYNC_ENABLED=false;
...
const sharedWebSyncPaused=()=>!root.AndroidNative&&!PUBLIC_WEB_BLABLA_SYNC_ENABLED;
const publicSyncPausedMessage='공유 웹 버전의 BlaBlaLink 공개 URL 연동은 현재 일시 중지되어 있습니다. 기존에 이 브라우저에 저장된 로스터는 계속 사용할 수 있습니다.';
...
async function rehydrateSnapshot(reason='앱 등록 캐릭터 재수화'){
  if(state.rehydrated&&reason==='부팅 재수화')return null;const snap=readJson(RAW_BASE,null);if(!snap?.characters?.length)return null;if(!installImporterWrapper())return null;const filtered=filteredRoster({nickname:snap.nickname,observedAt:snap.observedAt,characters:snap.characters});const ext=root.NIKKEV34610External,original=ext.__v34711OriginalImporter;if(!original)return null;purgeExternalSupplementals();const out=await original(filtered.roster,`${snap.source||snap.provider||'linked snapshot'} · V34.7.20 rehydrate`,snap.optionOverride||null);purgeExternalSupplementals();state.rehydrated=true;renderPolicySummary(snap.characters.length,out.imported,filtered.excluded.length,reason);try{root.dispatchEvent(new CustomEvent('nikke:v26-roster-updated',{detail:{source:'v34711-linked-snapshot-rehydrate',count:out.imported,updatedAt:new Date().toISOString()}}));}catch(_){}return{...out,received:snap.characters.length,excludedCount:filtered.excluded.length};
}
...
async function nativeOrWebSync(reason){
  if(sharedWebSyncPaused())throw new Error(publicSyncPausedMessage);
...
async function runScheduledIfDue(){if(sharedWebSyncPaused()||!hasProfile()||state.syncing)return null;const due=latestDueSlot(),rs=refreshState();if(rs.slots?.[due.id])return null;try{const out=await nativeOrWebSync(`목요일 ${due.hour===11?'오전 11시':'오후 7시'} 자동 갱신${Date.now()-due.epoch>30*60*1000?' · 지연 실행':''}`);const next=refreshState();next.slots={...(next.slots||{}),[due.id]:new Date().toISOString()};const keys=Object.keys(next.slots).sort().slice(-10);next.slots=Object.fromEntries(keys.map(key=>[key,next.slots[key]]));saveRefreshState(next);return out;}catch(_){return null;}}
function scheduleTimer(){if(state.timer)root.clearTimeout(state.timer);state.timer=0;if(sharedWebSyncPaused()||nativeScheduleAvailable()){updateRefreshPanel();return;}const next=nextSlot(),wait=Math.max(1000,next.epoch-Date.now()+1500);state.timer=root.setTimeout(()=>{runScheduledIfDue().finally(scheduleTimer);},Math.min(wait,2147480000));updateRefreshPanel();}
function updateRefreshPanel(message='',error=false){const panel=$('v34711RefreshPanel');if(!panel)return;const rs=refreshState(),next=nextSlot(),registered=appCatalog().length,eligible=calculationEligibleIds().size,snap=readJson(RAW_BASE,null),raw=arr(snap?.characters).length;const status=$('v34711RefreshStatus');if(status){status.classList.toggle('error',!!error);if(sharedWebSyncPaused()){status.innerHTML=`<b>공유 웹판 URL 연동 일시 중지</b><br>${esc(publicSyncPausedMessage)}<br>저장된 연동 원본 ${raw||'-'}명 · 앱 등록 ${registered}명 · 현재 계산 프로필 ${eligible}명`;return;}status.innerHTML=`${message?`<b>${esc(message)}</b><br>`:''}연동 원본 ${raw||'-'}명 · 앱 등록 ${registered}명 · 현재 계산 프로필 ${eligible}명<br>마지막 성공 ${rs.lastSuccess?esc(new Date(rs.lastSuccess).toLocaleString('ko-KR')):'없음'} · 다음 자동 ${esc(formatKst(next.epoch))}`;}}
function installRefreshUi(){if($('v34711RefreshPanel'))return true;const official=$('v34610BlaOfficial'),form=official?.closest('.ext-form');if(!form)return false;const paused=sharedWebSyncPaused(),button=root.document.createElement('button');button.id='v34711RefreshNow';button.type='button';button.textContent=paused?'공유 웹판 연동 일시 중지':'🔄 연동 데이터 새로고침';button.title=paused?publicSyncPausedMessage:'저장된 BlaBla 공개 링크로 즉시 최신 데이터를 다시 가져옵니다.';button.disabled=paused;form.appendChild(button);const panel=root.document.createElement('div');panel.id='v34711RefreshPanel';panel.className='notice';panel.style.marginTop='10px';panel.innerHTML=paused?`<b>V34.7.20 공유 웹 연동 상태</b> · 공개 URL 동기화 일시 중지<div id="v34711RefreshStatus" class="ext-status" style="margin-top:8px"></div>`:'<b>V34.7.20 연동 갱신</b> · 매주 목요일 11:00 / 19:00 KST 앱이 실행 중이면 자동 갱신하며, 꺼져 있던 경우 다음 실행 시 최신 미실행 슬롯을 즉시 보정합니다.<div id="v34711RefreshStatus" class="ext-status" style="margin-top:8px"></div>';form.insertAdjacentElement('afterend',panel);button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();requestManualRefresh().catch(()=>{});});const input=$('v34610BlaUrl'),server=$('v34610BlaServer');if(paused){for(const node of [input,server,official]){if(!node)continue;node.disabled=true;node.setAttribute('aria-disabled','true');node.title=publicSyncPausedMessage;}}for(const node of [input,server])node?.addEventListener('change',()=>{rememberInputs();scheduleTimer();});return true;}
function brand(){try{root.document.title='니케 성장 계산기 V34.7.20 · 동적 앱 등록 연산 · 주 2회 연동 갱신';for(const node of root.document.querySelectorAll('.footer,.footer-version'))node.textContent='Nikke Damage Growth Calculator · V34.7.20 · 2026-08-27';const button=$('nikke-kit-run');if(button&&!state.syncing)button.textContent='현재 앱 등록 로스터로 V34.7.20 5팀 계산';}catch(_){}}
function verify(){const registered=appCatalog().length,eligible=calculationEligibleIds().size,snap=readJson(RAW_BASE,null),result={version:VERSION,registered,calculationEligible:eligible,rawLinkedRows:arr(snap?.characters).length,hardcoded100:false,dynamicRegistration:true,manualRefresh:!!$('v34711RefreshNow'),publicWebSyncPaused:sharedWebSyncPaused(),weeklySlots:[11,19],timezone:'Asia/Seoul',importerWrapped:!!root.NIKKEV34610External?.__v34711FilteredImporter,optimizerFiltered:!!root.NIKKEV340FinalOptimizer?.__v34711AppRegistryFilter};result.pass=result.dynamicRegistration&&result.manualRefresh&&result.importerWrapped&&result.optimizerFiltered&&registered>=eligible;root.NIKKEV34711LinkedRosterRefreshVerification=Object.freeze(result);return result;}
async function install(){if(!root.NIKKE_V26_ROSTER_API||!root.NIKKEV34610External){root.setTimeout(install,100);return;}if(!readJson(RAW_BASE,null))reconstructLegacySnapshot();purgeExternalSupplementals();installImporterWrapper();installCalculationFilter();prefill();installNativeRefreshBridge();installRefreshUi();brand();if(!state.rehydrated)rehydrateSnapshot('부팅 재수화').catch(()=>{});scheduleTimer();verify();if(!state.installed){state.installed=true;root.addEventListener?.('focus',()=>{if(!sharedWebSyncPaused()&&!nativeScheduleAvailable())runScheduledIfDue();});root.document?.addEventListener?.('visibilitychange',()=>{if(root.document.visibilityState==='visible'&&!sharedWebSyncPaused()&&!nativeScheduleAvailable())runScheduledIfDue();});root.addEventListener?.('nikke:v26-roster-updated',()=>{installCalculationFilter();updateRefreshPanel();});if(!sharedWebSyncPaused()&&!nativeScheduleAvailable())root.setTimeout(runScheduledIfDue,1200);}}
