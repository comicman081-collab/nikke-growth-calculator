...
async function rehydrateSnapshot(reason='앱 등록 캐릭터 재수화'){
  if(state.rehydrated&&reason==='부팅 재수화')return null;const snap=readJson(RAW_BASE,null);if(!snap?.characters?.length)return null;if(!installImporterWrapper())return null;const filtered=filteredRoster({nickname:snap.nickname,observedAt:snap.observedAt,characters:snap.characters});const ext=root.NIKKEV34610External,original=ext.__v34711OriginalImporter;if(!original)return null;purgeExternalSupplementals();const out=await original(filtered.roster,`${snap.source||snap.provider||'linked snapshot'} · V34.7.12 rehydrate`,snap.optionOverride||null);purgeExternalSupplementals();state.rehydrated=true;renderPolicySummary(snap.characters.length,out.imported,filtered.excluded.length,reason);try{root.dispatchEvent(new CustomEvent('nikke:v26-roster-updated',{detail:{source:'v34711-linked-snapshot-rehydrate',count:out.imported,updatedAt:new Date().toISOString()}}));}catch(_){}return{...out,received:snap.characters.length,excludedCount:filtered.excluded.length};
}
...
function installRefreshUi(){if($('v34711RefreshPanel'))return true;const official=$('v34610BlaOfficial'),form=official?.closest('.ext-form');if(!form)return false;const button=root.document.createElement('button');button.id='v34711RefreshNow';button.type='button';button.textContent='🔄 연동 데이터 새로고침';button.title='저장된 BlaBla 공개 링크로 즉시 최신 데이터를 다시 가져옵니다.';form.appendChild(button);const panel=root.document.createElement('div');panel.id='v34711RefreshPanel';panel.className='notice';panel.style.marginTop='10px';panel.innerHTML='<b>V34.7.12 연동 갱신</b> · 매주 목요일 11:00 / 19:00 KST 앱이 실행 중이면 자동 갱신하며, 꺼져 있던 경우 다음 실행 시 최신 미실행 슬롯을 즉시 보정합니다.<div id="v34711RefreshStatus" class="ext-status" style="margin-top:8px"></div>';form.insertAdjacentElement('afterend',panel);button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();requestManualRefresh().catch(()=>{});});const input=$('v34610BlaUrl'),server=$('v34610BlaServer');for(const node of [input,server])node?.addEventListener('change',()=>{rememberInputs();scheduleTimer();});return true;}
function brand(){try{root.document.title='니케 성장 계산기 V34.7.12 · 동적 앱 등록 연산 · 주 2회 연동 갱신';for(const node of root.document.querySelectorAll('.footer,.footer-version'))node.textContent='Nikke Damage Growth Calculator · V34.7.12 · 2026-08-27';const button=$('nikke-kit-run');if(button&&!state.syncing)button.textContent='현재 앱 등록 로스터로 V34.7.12 5팀 계산';}catch(_){}}
