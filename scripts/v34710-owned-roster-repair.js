...
function render(result,mount,diagnostics){
 ...
 const note=root.document.createElement('div');note.className='notice ok';note.innerHTML=`<b>V34.7.11 실연동 로스터 5덱 복구 엔진</b><br>연동 ${diagnostics.receivedCount}명 · 보유 저장 ${diagnostics.ownedCount}명 · 계산 프로필 ${diagnostics.profileCount}명 · B1 ${diagnostics.b1Count} / B2 ${diagnostics.b2Count} / 활성 B3 ${diagnostics.b3Count} / FLEX ${diagnostics.flexCount} · ${esc(diagnostics.repairMode)} · ${(diagnostics.elapsedMs/1000).toFixed(1)}초`;mount.prepend(note);
}
...
async function run(ev){
 ...
 }finally{state.active=null;if(parts.panel)parts.panel.setAttribute('aria-busy','false');if(parts.button){parts.button.disabled=false;parts.button.textContent='현재 로스터로 V34.7.11 5팀 계산';}try{root.scrollTo(0,scrollY);}catch(_){}}})();
...
function brand(){try{root.document.title='니케 성장 계산기 V34.7.11 · 실연동 186명 로스터 5덱 복구';for(const node of root.document.querySelectorAll('.footer,.footer-version'))node.textContent='Nikke Damage Growth Calculator · V34.7.11 · 2026-08-27';}catch(_){}}
function install(){installDynamicOwnedApi();const parts=panelParts();if(parts.button){parts.button.disabled=false;parts.button.textContent=state.active?'계산 중지':'현재 로스터로 V34.7.11 5팀 계산';parts.button.dataset.v34710Repair='true';}root.NIKKEV3420Run=run;root.NIKKEV34710OwnedRosterRepair=Object.freeze({version:VERSION,run,calculate,verifyPropagation,getLastResult:()=>state.last,clearCache:()=>{state.cache=null;}});brand();}
