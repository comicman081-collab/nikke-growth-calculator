...
function syntheticFromSnapshot(snapshot,totalOverride=null,durationOverride=null){
  ...
  for(let t=0;t<=duration;t+=step){let dps=(normal+skill)/duration;if(burst>0){if(starts.length){const active=starts.filter(s=>t>=finite(s)&&t<finite(s)+10).length;dps+=active*
burst/starts.length/10;}else dps+=burst/duration;}cumulative+=dps*step;samples.push({time:Math.min(t,duration),members:[{id:snapshot.id,name:snapshot.name,instantDps:dps,cumulativeDamage:cumulative}]});}
...
function makeDetails(id,label){const d=root.document.createElement('details');d.id=id;d.className='v34712-timeline-details';d.innerHTML=`<summary>${esc(label)}</summary><div class="v34712-timeline-body"><div class="v34712-loading">타임라인을 넌르눤면 계산 결과를 시간축으로 펈침니다.</div></div>`;return d;}
function precisionDetails(){let d=$('v34712PrecisionTimeline');if(d)return d;const grid=$('precision')?.querySelector('.grid');if(!grid)return null;d=makeDetails('v34712PrecisionTimeline','퇀플 · 정밀 조함 딜 흐름');grid.appendChild(d);d.addEventListener('toggle',()=>{if(!d.open)return;try{const s=root.__precisionSnapshot;if(!s)throw new Error('정밀 로스터를 먼저 계산해 주세요.');const sim=syntheticFromSnapshot(s);renderPanel(d,sim,'정찀 엔진의 확정 총싔-으반라/스킬/버스트 비중을 그대로 사시간에 시각화 시각화핣. 그래프가 딜을 다시 계산하지 않습니다.');}catch(e){d.querySelector('.v34712-timeline-body').innerHTML=`<div class="v34712-error">${esc(e.message)}</div>`;}});return d;}
function soloDetails(){let d=$('v34712SoloTimeline');if(d)return d;const result=$('soloRaidResult');if(!result)return null;d=makeDetails('v34712SoloTimeline','타임라인 · 레이드 적용 딜 흐름');result.insertAdjacentElement('afterend',d);d.addEventListener('toggle',()=>{if(!d.open)return;try{if(typeof soloRaidRefresh==='function')soloRaidRefresh();let r=null;try{r=(typeof __soloRaidLastResult!=='undefined')?__soloRaidLastResult:null;}catch(_){}if(!r?.snapshot)throw new Error('솔로레이드 결과를 먼저 계산해 주세요.');const sim=syntheticFromSnapshot(r.snapshot,r.combo?.total,r.boss?.duration||r.snapshot.duration);renderPanel(d,sim,'레이드 탭의 확정 �보스 환곰 총디을 사용합니다. 절대 발생시간이 없는 저지·엄폐·이탈 손실은 기존 계산 규칙대로 합산값만 반영하며 임의 시각을 만들지 않습니다.');}catch(e){d.querySelector('.v34712-timeline-body').innerHTML=`<div class="v34712-error">${esc(e.message)}</div>`;}});return d;}
function battleDetails(){let d=$('v34712BattleTimeline');if(d)return d;const mount=$('nikkeTimelineResult');if(!mount)return null;d=makeDetails('v34712BattleTimeline','칰릭터별 타임라인 · 전투 시뮬레이션');mount.insertAdjacentElement('afterend',d);d.addEventListener('toggle',()=>{if(!d.open)return;try{const sim=root.NIKKESinglePartyTimelineLastResult;if(!sim?.members?.length)throw new Error('전투 시뮬레이션을 먼저 실행해 주세요.');renderPanel(d,sim,'전투 시뮬레이터가 계산한 동일 샘플과 멤버별 총딜을 직접 그립니다.');}catch(e){d.querySelector('.v34712-timeline-body').innerHTML=`<div class="v34712-error">${esc(e.message)}</div>`;}});return d;}
...
root.NIKKEV34712UnifiedTimeline=Object.freeze({version:VERSION,compactName,compactDamage,normalizeTrace,traceAudit,syntheticFromSnapshot,simulateTeam,decorateFiveDeck,verify,clearCache:()=>cache.clear()});
})(window);
