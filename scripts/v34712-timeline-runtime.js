...
function updateBrand(){try{root.document.title=root.document.title.replace(/V34\.7\.(?:11|12)/g,'V34.7.16');for(const node of root.document.querySelectorAll('.footer,.footer-version'))node.textContent=String(node.textContent||'').replace(/V34\.7\.(?:11|12)/g,'V34.7.16');}catch(_){}}
