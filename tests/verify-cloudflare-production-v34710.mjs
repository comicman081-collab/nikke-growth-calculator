...
for(let attempt=1;attempt<=90;attempt+=1){try{const result=await textResponse(`${BASE}/?_v34711=${encodeURIComponent(process.env.GITHUB_SHA||'manual')}-${attempt}`,{headers:{'cache-control':'no-cache','user-agent':'nikke-v34711-live-verifier/1.1'}});status=result.response.status;headers=Object.fromEntries(result.response.headers.entries());html=result.text;if(result.response.ok&&html.includes('V34.7.12')&&html.includes('id="v34711-linked-roster-refresh"'))break;last=`attempt ${attempt}: HTTP ${result.response.status}; version=${html.includes('V34.7.12')}; marker=${html.includes('id="v34711-linked-roster-refresh"')}`;}catch(error){last=`attempt ${attempt}: ${error.message}`;}if(attempt<90)await delay(8000);}
assert.ok(html.includes('V34.7.12'),`Cloudflare production did not expose V34.7.12: ${last}`);
...
const common={Origin:BASE,Referer:`${BASE}/`,'cache-control':'no-cache','user-agent':'nikke-v34711-live-verifier/1.1'};
...
fs.writeFileSync(REPORT,JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));console.log(`V34.7.12 Cloudflare production HTML + Worker bridge probe + refresh UI verification: PASS; live fixture=${liveFixture.status}`);
